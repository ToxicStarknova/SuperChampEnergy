...
class ToolTip:
    ...
    def show_tip(self, event=None):
        ...
        label = tk.Label(tw, text=self.text, justify=tk.LEFT,
                         ...
                         font=("Helvetica", 9, "normal"), padding=6, wraplength=320)
        label.pack()
...
class HomeBatteryCalculatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Home Battery & Tariff Optimization Tool - V14.1 (All Strategies & Console Scroll)")
    ...
    def setup_ui(self):
        ...
        # 3. Strategy Explanations with Tooltips
        explainer_frame = ttk.LabelFrame(left_panel, text=" 3. Charging Strategies (Hover for info) ", padding=10)
        explainer_frame.pack(fill=tk.X, pady=(0, 10))
        
        strategies_info = [
            ("• Self-Consumption", "Prioritizes storing excess solar production locally. The battery will never charge using grid power, acting strictly as a solar sponge. Good baseline comparison strategy."),
            ("• Import-Minimiser", "Force-charges the home battery system up to max capacity during the lowest cost daily tariff window to reliably cover your daytime load profiles."),
            ("• Export-Maximiser", "Forces a proactive battery energy dump directly to the grid in the 4 hours immediately prior to the cheap grid window starting, clearing maximum volume space to collect cheap night power."),
            ("• Balanced-Export", "Runs aggressive arbitrage/grid-dump protocols during spring and summer months, but bypasses the pre-charge grid dump during winter season (Nov-Feb) to ensure home heating security."),
            ("• Import-Minimiser (Summer Pass)", "Prevents solar generation from charging the battery between March and October to bypass AC/DC conversion rounds. Feeds solar to the grid at 100% efficiency instead.")
        ]
        
        for label_text, tip_text in strategies_info:
            lbl = ttk.Label(explainer_frame, text=label_text, foreground="#4f46e5", font=("Helvetica", 9, "underline", "bold"), cursor="hand2")
            lbl.pack(anchor=tk.W, pady=2)
            ToolTip(lbl, tip_text)

        # 4. Telemetry Output (Now Scrollable)
        ...
        self.txt_stats = tk.Text(console_container, height=6, bg="#f8fafc", foreground="#334155", 
    ...
    def update_console(self, text_string, color_hex="#334155"):
        ...
        self.txt_stats.config(foreground=color_hex, state=tk.DISABLED)
        self.root.update()
    ...
    def run_sweep(self):
        ...
        try:
            ...
            # Updated strategies loop array to evaluate ALL 5 metrics
            ...
            for _, row in valid_tariffs.iterrows():
                ...
                baseline_import_costs, base_limit_exceeded = _calc_cost_with_overage(orig_imports, import_prices.values, is_ev_window, ev_overage_rate, months_array)
                ...
                for strategy in all_strategies:
                    ...
                    strategy_import_costs, strat_limit_exceeded = _calc_cost_with_overage(imports, import_prices.values, is_ev_window, ev_overage_rate, months_array)
            ...
            for dyn in dynamic_suppliers:
                ...
                baseline_import_costs, base_limit_exceeded = _calc_cost_with_overage(orig_imports, import_prices.values, dyn_is_ev_window, dyn_ev_overage_rate, months_array)
                annual_imp_base = np.sum(baseline_import_costs)
                annual_exp_base = np.sum(orig_exports * fit_rate)
                net_bill_base = (annual_imp_base - annual_exp_base) * scaling_factor + fixed_charges
                ...
                for strategy in all_strategies:
                    ...
                    strategy_import_costs, strat_limit_exceeded = _calc_cost_with_overage(imports, import_prices.values, dyn_is_ev_window, dyn_ev_overage_rate, months_array)
                    if strat_limit_exceeded:
