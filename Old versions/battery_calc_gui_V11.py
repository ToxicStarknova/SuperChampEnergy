...
def prepare_dam(hdf_idx, dam_file):
    df_dam = pd.read_excel(dam_file)
    if 'auction' in df_dam.columns:
        df_dam = df_dam[df_dam['auction'] == 'DAM']
    
    dt_series = pd.to_datetime(df_dam['timestamp'])
...
def run_simulation(df_hdf, import_prices, export_price, strategy, force_charge_hours, params):
    ...
    strategy_map = {'self-consumption': 0, 'import-minimiser': 1, 'export-maximiser': 2, 'balanced-export-maximiser': 3, 'import-minimiser-summer-pass': 4}
    
    # Broadcast fixed 24h pattern to length N
...
def run_dynamic_simulation(df_hdf, import_prices, export_price, strategy, params):
    ...
    # Dynamically find the absolute cheapest 6 slots per calendar day
    ...
    if strategy in ['export-maximiser', 'balanced-export-maximiser']:
        for i in range(len(force_charge_mask)):
            if force_charge_mask[i]:
                start_idx = max(0, i - 8) # 4 hours before
...
class HomeBatteryCalculatorApp:
    ...
    def setup_ui(self):
        ...
        self.entry_charge_rate = ttk.Entry(params_frame, width=8); self.entry_charge_rate.insert(0, "18.0"); self.entry_charge_rate.grid(row=1, column=1, sticky=tk.W, pady=4, padx=5)
        ...
        explainer_text = (
            "• Self-Consumption: Prioritizes storing excess solar. Never force-charges from the grid.\n\n"
            "• Import-Minimiser: Force-charges the battery during cheapest daily window to cover daytime load.\n\n"
            "• Import-Minimiser (Summer Pass): Prevents solar from charging the battery from Mar-Oct to bypass AC conversion losses.\n\n"
            "• Balanced-Export: Dumps battery to grid during peak hours to maximize Export profit."
        )
        ...
        self.lbl_stats = ttk.Label(self.stats_frame, text="Waiting for simulation...\n\n\n\n", foreground="#6b7280", font=("Consolas", 9), justify=tk.LEFT)
        ...
        self.tree.heading("rank", text="#"); self.tree.heading("supplier", text="Supplier"); self.tree.heading("tariff", text="Tariff Name")
        self.tree.heading("strategy", text="Winning Strategy"); self.tree.heading("arbitrage", text="Arb. Viable?")
        self.tree.heading("import", text="Import Cost (€)"); self.tree.heading("export", text="Export FIT (€)"); self.tree.heading("bill", text="Annual Bill (€)")

        self.tree.column("rank", width=40, anchor=tk.CENTER); self.tree.column("supplier", width=110, anchor=tk.W)
        self.tree.column("tariff", width=220, anchor=tk.W); self.tree.column("strategy", width=200, anchor=tk.CENTER)
        self.tree.column("arbitrage", width=80, anchor=tk.CENTER); self.tree.column("import", width=100, anchor=tk.E)
        self.tree.column("export", width=100, anchor=tk.E); self.tree.column("bill", width=110, anchor=tk.E)
    ...
    def open_custom_tariff_dialog(self):
        ...
        dlg.title("Add Custom Tariff"); dlg.geometry("450x450"); dlg.grab_set() 
        ...
        ev_frame = ttk.Frame(frame); ev_frame.grid(row=9, column=0, columnspan=2, sticky=tk.W, pady=10)
        ...
        def save_tariff():
            try:
                new_tariff = {
                    ...
                    'Fit unit': float(ent_fit.get() or 0.0), 'Supply Region': self.combo_region.get().strip().lower(), 
    ...
    def export_simulated_hdf(self, tab_idx):
        ...
        internal_id = tab_ui['internal_id']; strategy = tab_ui['strategy']
        ...
        filename = filedialog.asksaveasfilename(defaultextension=".csv", initialfile=default_filename, filetypes=[("CSV files", "*.csv")])
        ...
        try:
            ...
            df_imp = pd.DataFrame({'timestamp': times, 'MPRN': self.mprn, 'Meter Serial Number': self.meter_serial,
                                   'Read Value': sim_data['import'].round(3), 'Read Type': 'Active Import Interval (kWh)', 'Read Date and End Time': dt_strs})
            df_exp = pd.DataFrame({'timestamp': times, 'MPRN': self.mprn, 'Meter Serial Number': self.meter_serial,
                                   'Read Value': sim_data['export'].round(3), 'Read Type': 'Active Export Interval (kWh)', 'Read Date and End Time': dt_strs})

            df_out = pd.concat([df_imp, df_exp]).sort_values(by=['timestamp', 'Read Type'], ascending=[False, False]).drop(columns=['timestamp'])
            df_out = df_out[['MPRN', 'Meter Serial Number', 'Read Value', 'Read Type', 'Read Date and End Time']]
    ...
    def run_sweep(self):
        ...
        self.lbl_stats.config(text="Parsing Data...\n\n\n\n", foreground="#f59e0b"); self.root.update()
        ...
        try:
            ...
            if valid_tariffs.empty: raise ValueError("No valid tariffs found for the selected region.")
            ...
            for _, row in valid_tariffs.iterrows():
                ...
                fixed_charges = float(row['Standing charge']) + float(row['PSO Levy']) - (float(row['Cash bonus']) if not pd.isna(row['Cash bonus']) else 0.0)

                # Baseline
                annual_imp_base = np.sum(orig_imports * import_prices.values); annual_exp_base = np.sum(orig_exports * fit_rate)
                ...
                results.append({
                    ...
                    'Arbitrage': "N/A", 'Import': annual_imp_base, 'Export': annual_exp_base, 'Bill': annual_imp_base - annual_exp_base + fixed_charges, '_id': tid, 'is_dynamic': False
                })
                ...
                for strategy in ['self-consumption', 'import-minimiser', 'import-minimiser-summer-pass', 'balanced-export-maximiser']:
                    ...
                    net_bill = np.sum(imports * import_prices.values) - np.sum(exports * fit_rate)
                    results.append({'Supplier': row['Supplier'], 'Tariff': row['Tariff name'], 'Strategy': strategy, 'Arbitrage': "Yes" if is_arb else "No", 
                                    'Import': np.sum(imports * import_prices.values), 'Export': np.sum(exports * fit_rate), 'Bill': net_bill + fixed_charges, '_id': tid, 'is_dynamic': False})

            # 2. Evaluate Dynamic Tariffs
            for dyn in dynamic_suppliers:
                fit_rate = dyn['Fit unit'] / 100.0
                fixed_charges = dyn['Standing charge']
                ...
                results.append({
                    ...
                    'Arbitrage': "N/A", 'Import': annual_imp_base, 'Export': annual_exp_base, 'Bill': annual_imp_base - annual_exp_base + fixed_charges, '_id': tid, 'is_dynamic': True
                })
                ...
                for strategy in ['self-consumption', 'import-minimiser', 'import-minimiser-summer-pass', 'balanced-export-maximiser']:
                    ...
                    self.detailed_results[tid][strategy] = {'import': imports, 'export': exports, 'soc': soc}
                    net_bill = np.sum(imports * import_prices.values) - np.sum(exports * fit_rate)
                    results.append({'Supplier': dyn['Supplier'], 'Tariff': dyn['Tariff name'], 'Strategy': strategy, 'Arbitrage': "Variable", 
                                    'Import': np.sum(imports * import_prices.values), 'Export': np.sum(exports * fit_rate), 'Bill': net_bill + fixed_charges, '_id': tid, 'is_dynamic': True})

            # UI Update and Savings
            calc_time = time.time() - start_time
            telemetry = (
                f"[✓] Data Points: {total_rows:,} (Half-hourly)\n"
                f"[✓] Tariffs Evaluated: {num_tariffs}\n"
                f"[✓] Total Simulations: {num_tariffs * 5:,}\n"
                f"⚡ C-Compiled OPs: ~{total_rows * num_tariffs * 4 * 18:,}\n"
            ...
            # Filter leaderboard: Optimized + Single Best Baseline
