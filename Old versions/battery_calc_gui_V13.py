...
def get_half_hourly_rates_for_row(row, date_range):
    ...
    return pd.Series(prices, index=date_range), np.array(is_ev_window, dtype=np.bool_), ev_overage_rate
...
@njit
def _calc_cost_with_overage(imports, prices, is_ev_window, ev_overage_rate, months):
    """Calculates final billing cost while tracking bimonthly 1000kWh limit."""
    n = len(imports)
    cost = np.zeros(n)
    ...
    for i in range(n):
        ...
        if is_ev_window[i]:
            # Map month (1-12) to bimonthly period (0-5)
...
class HomeBatteryCalculatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Home Battery & Tariff Optimization Tool - V13 (Overage Tracking)")
    ...
    def run_sweep(self):
        ...
        try:
            ...
            for _, row in valid_tariffs.iterrows():
                ...
                import_prices, is_ev_window, ev_overage_rate = get_half_hourly_rates_for_row(row, self.df_hdf.index)
                ...
                # Baseline Control (Calculated with the EV Overage rule)
                baseline_import_costs = _calc_cost_with_overage(orig_imports, import_prices.values, is_ev_window, ev_overage_rate, months_array)
                ...
                for strategy in ['self-consumption', 'import-minimiser', 'import-minimiser-summer-pass', 'balanced-export-maximiser']:
                    ...
                    # Compute new strategy cost explicitly running through the bimonthly tracker
                    strategy_import_costs = _calc_cost_with_overage(imports, import_prices.values, is_ev_window, ev_overage_rate, months_array)
                    annual_imp_cost = np.sum(strategy_import_costs)
                    annual_exp_rev = np.sum(exports * fit_rate)
                    
                    net_bill = annual_imp_cost - annual_exp_rev
                    results.append({'Supplier': row['Supplier'], 'Tariff': row['Tariff name'], 'Strategy': strategy, 'Arbitrage': "Yes" if is_arb else "No", 
                                    'Import': annual_imp_cost, 'Export': annual_exp_rev, 'Bill': net_bill + fixed_charges, '_id': tid, 'is_dynamic': False})
            ...
            for dyn in dynamic_suppliers:
                ...
                # Dynamic plans don't typically have EV fair usage limits, so we feed an empty mask
                ...
                baseline_import_costs = _calc_cost_with_overage(orig_imports, import_prices.values, dyn_is_ev_window, dyn_ev_overage_rate, months_array)
                annual_imp_base = np.sum(baseline_import_costs)
                annual_exp_base = np.sum(orig_exports * fit_rate)
                ...
                for strategy in ['self-consumption', 'import-minimiser', 'import-minimiser-summer-pass', 'balanced-export-maximiser']:
                    ...
                    strategy_import_costs = _calc_cost_with_overage(imports, import_prices.values, dyn_is_ev_window, dyn_ev_overage_rate, months_array)
                    annual_imp_cost = np.sum(strategy_import_costs)
                    annual_exp_rev = np.sum(exports * fit_rate)
                    
                    net_bill = annual_imp_cost - annual_exp_rev
                    results.append({'Supplier': dyn['Supplier'], 'Tariff': dyn['Tariff name'], 'Strategy': strategy, 'Arbitrage': "Variable", 
                                    'Import': annual_imp_cost, 'Export': annual_exp_rev, 'Bill': net_bill + fixed_charges, '_id': tid, 'is_dynamic': True})
