...
class HomeBatteryCalculatorApp:
    ...
    def setup_ui(self):
        ...
        self.tree.column("tariff", width=220, anchor=tk.W)
        self.tree.column("strategy", width=200, anchor=tk.CENTER)
        ...
        self.tree.tag_configure('best_baseline', background='#ffedd5', foreground='#b45309') # Amber highlight for best unoptimized
        self.tree.tag_configure('baseline', background='#f3f4f6', foreground='#9ca3af') # Grayed out for other unoptimized
    ...
    def run_sweep(self):
        ...
        try:
            ...
            # Original usage arrays for Baseline Unoptimized testing
            ...
            for _, row in valid_tariffs.iterrows():
                ...
                # --- 1. Baseline Calculation (No Battery Shift) ---
                annual_imp_base = np.sum(orig_imports * import_prices.values)
                annual_exp_base = np.sum(orig_exports * fit_rate)
                base_bill = annual_imp_base - annual_exp_base + fixed_charges
                ...
                results.append({
                    ...
                    'Arbitrage': "N/A", 'Import': annual_imp_base, 'Export': annual_exp_base, 'Bill': base_bill, '_id': tid
                })

                # --- 2. Optimized Loop Calculations ---
                for strategy in ['self-consumption', 'import-minimiser', 'import-minimiser-summer-pass', 'balanced-export-maximiser']:
                    ...
                    results.append({
                        ...
                        'Bill': net_bill + fixed_charges, '_id': tid
                    })
            ...
            max_savings = best_base_bill - best_opt_bill
            
            # Update Dashboard Label
            self.lbl_savings.config(
                text=f"💰 System Optimization Savings: €{max_savings:,.2f} / yr  |  Cheapest Unoptimized Plan: €{best_base_bill:,.2f}"
            )

            self.leaderboard_data = df_res.sort_values(by='Bill').reset_index(drop=True)
            
            for item in self.tree.get_children(): self.tree.delete(item)
            for idx, row in self.leaderboard_data.iterrows():
                tags = ()
                if row['Strategy'] == 'baseline-no-battery':
                    # Check if this row is the absolute cheapest baseline mathematically
                    if abs(row['Bill'] - best_base_bill) < 0.01:
                        tags = ('best_baseline',)
                    else:
                        tags = ('baseline',)
            ...
            # Populate Top 3 Tabs (Filters out unoptimized baselines from showing up in the graph tabs)
            optimized_leaderboard = df_res[~baseline_mask].sort_values(by='Bill').head(3).reset_index(drop=True)
            
            for i, (_, row) in enumerate(optimized_leaderboard.iterrows()):
                if i >= 3: break
