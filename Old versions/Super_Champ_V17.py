...
class ToolTip:
    ...
    def show_tip(self, event=None):
        ...
        tw.attributes("-topmost", True)  # Ensure it renders on the top layer
...
def clean_pre_arbitraged_data(df, is_already_battery_owner=False):
    """
    Cleans incoming ESB HDF data if the user already has an active battery system.
    Removes phantom night exports and flattens grid-charging spikes back to standard baseloads.
    """
    if not is_already_battery_owner:
        return df
    
    cleaned_df = df.copy()
    hour_array = cleaned_df.index.hour
    month_array = cleaned_df.index.month
    
    # 1. Eliminate Phantom Night Export (Solar Gating Rule)
    # In Ireland, solar generation between 10 PM and 5 AM is physically impossible.
    night_mask = (hour_array >= 22) | (hour_array < 5)
    cleaned_df.loc[night_mask, 'generation'] = 0.0
    
    # 2. Flatten Grid-Charging Spikes (Baseload Reconstruction)
    for month in range(1, 13):
        month_mask = (month_array == month)
        
        # Look for baseline consumption during non-charging night intervals
        non_charging_night = month_mask & night_mask & (cleaned_df['consumption'] < 1.0)
        if non_charging_night.any():
            baseload_proxy = cleaned_df.loc[non_charging_night, 'consumption'].median()
        else:
            baseload_proxy = 0.25  # 500W fallback proxy
            
        if pd.isna(baseload_proxy) or baseload_proxy > 0.5:
            baseload_proxy = 0.25
            
        # Overwrite any heavy night charging spike (>1.5 kWh per half hour) with baseline proxy
        charging_event_mask = month_mask & night_mask & (cleaned_df['consumption'] > 1.5)
        cleaned_df.loc[charging_event_mask, 'consumption'] = baseload_proxy
...
class HomeBatteryCalculatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Home Battery & Tariff Optimization Tool - V14.3 (De-Arbitrage Feature)")
        ...
        self.is_battery_owner = tk.BooleanVar(value=False)
    ...
    def setup_ui(self):
        ...
        # Added De-Arbitrage HDF Flag Selector Checkbox row here
        tk.Checkbutton(params_frame, text="ESB HDF contains battery load? (Filter pre-existing hardware arbitrage rules)", 
                       variable=self.is_battery_owner, font=("Helvetica", 9, "italic"), foreground="#4f46e5").grid(row=5, column=0, columnspan=4, sticky=tk.W, pady=(6, 2))

        # 3. Strategy Explanations with Fixed Hover Tooltips
    ...
    def run_sweep(self):
        ...
        self.update_console("Parsing Input Data & Scrubbing Pre-existing Hardware Tracks...", "#f59e0b")

        try:
            start_time = time.time()
            raw_hdf, mprn_val, meter_val = parse_hdf(self.hdf_path.get().strip())
            
            # Apply the De-Arbitrage Filter to clear pre-existing hardware cycles if checked
            raw_hdf = clean_pre_arbitraged_data(raw_hdf, is_already_battery_owner=self.is_battery_owner.get())
            ...
            if self.is_battery_owner.get():
                telemetry += f"\n⚙️ De-Arbitrage: Scrubbed pre-existing hardware cycles from loaded profile successfully."
