...
class HomeBatteryCalculatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Home Battery & Tariff Optimization Tool - V14 (Extended Data & Export)")
        self.root.geometry("1700x880")
    ...
    def export_leaderboard(self):
        ...
        if filepath:
            try:
                ...
                export_df = export_df.drop(columns=['_id', 'is_dynamic']) # Clean up internal IDs before saving
    ...
    def export_simulated_hdf(self, local_idx):
        ...
        try:
            # Shift timestamps back by 30 mins to get the HDF interval end time
            end_times = self.df_hdf.index + pd.Timedelta(minutes=30)
            formatted_times = end_times.strftime('%d-%m-%Y %H:%M')
            
            # Extract simulated import/export values (in kWh)
            ...
            # Alternate Import and Export rows to match the ESB Networks format
            rows = []
            ...
            # Write HDF header matching ESB Networks template
    ...
    def run_sweep(self):
        ...
        try:
            ...
            for dyn in dynamic_suppliers:
                ...
                import_prices = import_prices * 1.09  # Add 9% VAT
            ...
            if is_short_duration:
                telemetry += f"\n⚠️ Short Data: {num_days} days (Scaled by {scaling_factor:.2f}x)"
            
            ev_exceeded_names = sorted(list(set([p.split(' (')[0] for p in exceeded_plans])))
            if ev_exceeded_names:
                telemetry += f"\n⚠️ EV Cap Exceeded: {', '.join(ev_exceeded_names[:2])}"
                if len(ev_exceeded_names) > 2:
                    telemetry += f" (+{len(ev_exceeded_names)-2} more)"
